...
test('No hardcoded colors in components', () => {
  const css = readCssFile('_components.css');
  // Allow hex colors only in comments or specific legacy cases
  const colorMatches = css.match(/#[0-9a-f]{3,6}(?!.*(\/\/|comment|legacy))/gi) || [];
  // Filter out any found in comments
  const nonCommentMatches = colorMatches.filter(match => {
    const idx = css.indexOf(match);
    const before = css.substring(Math.max(0, idx - 50), idx);
    return !before.includes('/*') && !before.includes('//');
  });
  
  // Allow very minimal hardcoded colors (form element specifics)
  assert(nonCommentMatches.length < 5, `Too many hardcoded colors found: ${nonCommentMatches.join(', ')}`);
});
...
test('Container max-width set to 1440px', () => {
  const css = readCssFile('_layout.css');
